...
class FeatureFactory:
    ...
    def computeFeatures(self, words, previousLabel, position):
        ...
        # check the previous two words
        if position > 1:
            prevWord = words[position - 1]
        else:
            prevWord = '<BOS>'
        if position > 2:
            twoBackWord = words[position - 2]
        else:
            twoBackWord = '<BOS>'

        #check the starting letter of the word
        firstLetter = words[position][0]
	...
	""" TODO: Add your features here """
        ...
        features.append("twoBackWord=" + twoBackWord)
        features.append("firstLetter=" + firstLetter)
